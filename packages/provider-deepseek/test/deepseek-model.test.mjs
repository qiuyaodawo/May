...
test("streams reasoning, text, usage, and sends DeepSeek configuration", async () => {
  let capturedUrl;
  let capturedInit;
...
test("round-trips reasoning_content through a May tool-call loop", async () => {
  ...
  const fetch = async (_url, init) => {
    ...
    assert.ok(response, "unexpected extra DeepSeek request");
    return response;
  };
  const model = new DeepSeekModel({
    apiKey: "secret",
...
test("rejects malformed and incomplete DeepSeek streams", async (t) => {
  ...
  await t.test("invalid JSON", () =>
    rejectStream(["{"], DeepSeekProtocolError, "DEEPSEEK_PROTOCOL_ERROR"));
  ...
  await t.test("length finish reason", () =>
    rejectStream([
      ...
      "[DONE]",
    ], DeepSeekFinishReasonError, "DEEPSEEK_INCOMPLETE_RESPONSE"));

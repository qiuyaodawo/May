...
export function resolveMaybeCodeSkillDirectories(config: MayConfig, workspace: string): readonly string[] | false {
  ...
  if (typeof value !== "object" || value === null || Array.isArray(value) || Object.keys(value).some((key) => key !== "directories")) throw new Error("apps.maybecode.skills must be false or {directories: string[]}");
  if (!("directories" in value)) return defaultMaybeCodeSkillDirectories(workspace);
  if (!Array.isArray(value.directories) || value.directories.length > 32 || value.directories.some((path) => typeof path !== "string" || path.trim() === "")) throw new Error("skills.directories must contain at most 32 non-empty paths");

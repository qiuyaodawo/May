...
import {
  createMaybeCodeToolRendererRegistry,
...
export class TranscriptView implements InteractiveComponent, FocusTarget {
  ...
  constructor(
    ...
    this.toolRenderers = options.toolRenderers ?? createMaybeCodeToolRendererRegistry();
  }
    ...
    switch (this.item.kind) {
      ...
      case "tool":
        return new Text(options.toolRenderers.render(this.item, {
          ...
          theme: options.theme,
          detailsHint: options.selected ? "Enter details" : "Ctrl+X D all",
        })).render(size);
      case "user":
        return new Text(renderUser(this.item, options.theme)).render(size);
      case "approval":
...
function normalizeOptions(options: TranscriptViewOptions): TranscriptItemViewOptions {
  return {
    ...
    theme: options.theme ?? MAYBECODE_DARK_THEME,
    toolRenderers: options.toolRenderers ?? createMaybeCodeToolRendererRegistry(),
  };
}
  ...
  if (value !== "") {
    children.push(new Text(
      `${styleText("◆", options.theme.accent)} ` +
        styleText("MaybeCode", options.theme.assistantLabel),
    ));
...
function renderUser(item: UserTranscriptItem, theme: TuiTheme): string {
  const safe = sanitizeTerminalText(item.text);
  return `${styleText("›", theme.accent)} ${styleText("You", theme.userMessage)}\n` +
    styleLines(indent(safe), theme.text);
}
